window.onload = function init() {
   ...
   for (let i = 0; i < 6; i++) { kerningTextureArr[i].side = THREE.BackSide; }
    ...
    // 초기 화면 장애물 선언
    const primaryObstract1Geo = new THREE.BoxGeometry(100, 100, 100); // 큐브의 크기를 설정
    const primaryObstract1Mat = new THREE.MeshBasicMaterial({ color: 0x00ff00 }); // 큐브의 재질 및 색상 설정
    const primaryObstract1 = new THREE.Mesh(primaryObstract1Geo, primaryObstract1Mat); // 큐브 생성

    // 장면에 추가
    scene.add(primaryObstract1);

    // 초기 화면 장애물 위치 조정
    primaryObstract1.position.x = 16.5;
    primaryObstract1.position.y = 40;
    primaryObstract1.position.z = -1150;

    // 유진아 내가 장애물 큐브는 만들어놨는데 지피티가 준 함수에 넣어서 실행은 너가 해 화이팅! 6시 12분의 유진이가
